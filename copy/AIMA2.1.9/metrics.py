...
def compute_rep_metrics(
    ...
    df_angles — DataFrame с колонкой angle_col (обычно KneeAngle_fused_filt_deg).
    """
    ...
    # УБИРАЕМ МЕДЛЕННЫЙ ТРЕНД, чтобы не было «ползущей» линии
    angle = _detrend_moving_average(
        raw_angle,
        samplerate_hz=float(samplerate_hz),
        window_sec=5.0,   # можно потом поиграться: 4–8 сек
    )
    ...
    # Границы репов: середины между соседними пиками
