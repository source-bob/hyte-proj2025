...
from angles import compute_quaternions, compute_knee_angles_pca
from metrics import detect_reps_gyro, compute_rep_metrics, filter_squat_reps
...
def main():
    ...
    df_angles = compute_knee_angles_pca(df_proc, quats)
    ...
    print(f"\n[MAIN] Найдено сегментов ({movement_class}): {len(peaks)}")
    print(f"[MAIN] Индексы пиков: {peaks}")
    ...
    print(df_metrics)
        ...
        # --- Фильтрация 'полноценных' приседаний для SQUAT ---
    df_metrics_for_features = df_metrics
    if movement_class == "squat":
        df_metrics_for_features = filter_squat_reps(
            df_metrics,
            min_angle_deg=20.0,
            min_dur_sec=1.0,
        )
    ...
    print(
        f"\n[INFO] Сегментов по сигналу (Gyro-пики): {len(df_metrics)} | "
        f"'Полноценных' приседаний по глубине и длительности: {len(df_metrics_for_features)}"
    )


    # --- 7. Фичи сессии ---
    session_feats = session_features_from_metrics(
        df_metrics_for_features,
        df_angles,
        samplerate_hz=MOVESENSE_SAMPLERATE_HZ,
    )
    ...
    try:
        ...
        cnn_res = classify_reps_cnn(df_proc, df_metrics_for_features, model, device)
